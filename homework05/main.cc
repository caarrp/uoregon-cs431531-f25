...
void spmv_coo_naive(int world_rank, int world_size, 
					...
					double** res, double timer[])
{
	...
	if (world_rank == 0){//rank 0 deals with remander
		int work_per_node = nnz/world_size;
		nnz_per_rank = work_per_node;

		if (nnz % world_size != 0){//then deal w remainder
			//int rem = nnz % world_size;
			//work_per_node += rem;
			nnz_per_rank += 1;
		}
	}
	...
	// TODO
	//fprintf(stdout, "Rank %d: Before nnz bcast: nnz=%d (addr=%p)\n", 
        //world_rank, nnz, (void*)&nnz);
	//fprintf(stdout, "Rank %d: Before nnz_per_rank bcast: nnz_per_rank=%d (addr=%p)\n",
        //world_rank, nnz_per_rank, (void*)&nnz_per_rank);

	MPI_Bcast(&nnz, 1, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Bcast(&nnz_per_rank, 1, MPI_INT, 0, MPI_COMM_WORLD);

	//fprintf(stdout, "Rank %d: After nnz bcast: nnz=%d\n", world_rank, nnz);
	//fprintf(stdout, "Rank %d: After nnz_per_rank bcast: nnz_per_rank=%d\n",
        //world_rank, nnz_per_rank);
    // broadcasting m and n
	// TODO 
	//fprintf(stdout, "Rank %d: Before m bcast: m=%d (addr=%p)\n", 
        //world_rank, m, (void*)&m);
	//fprintf(stdout, "Rank %d: Before n bcast: n=%d (addr=%p)\n",
        //world_rank, n, (void*)&n);
	
	MPI_Bcast(&m, 1, MPI_INT, 0, MPI_COMM_WORLD);
	MPI_Bcast(&n, 1, MPI_INT, 0, MPI_COMM_WORLD);
	
	//fprintf(stdout, "Rank %d: After m bcast: m=%d\n", world_rank, m);
	//fprintf(stdout, "Rank %d: After n bcast: n=%d\n",
        //world_rank, n);
	// Lastly, broadcast the input vector x
	// TODO
	//fprintf(stdout, "Rank %d: Before malloc - n=%d, bytes=%ld\n", 
 	       //world_rank, n, (long)n * sizeof(double));
	//fflush(stdout);
	
	if (world_rank != 0) {
    		vector_x = (double*)malloc(n * sizeof(double));
	 }
	 //fprintf(stdout, "Rank %d: before vector bcast\n", world_rank);
	...
	//fprintf(stdout, "node %d : scatter success\n", world_rank);
