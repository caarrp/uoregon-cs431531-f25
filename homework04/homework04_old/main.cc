...
int main(int argc, char** argv)
{
    ...
    fprintf(stdout, "Executing GPU CSR SpMV allocation complete\n");
    t0 = ReadTSC();
    // spmv_gpu_2(drp, dci, dv, m, n, nnz, dx, db);
    spmv_gpu(drp, dci, dv, m, n, nnz, dx, db);
    timer[GPU_SPMV_TIME] += ElapsedTime(ReadTSC() - t0);

    fprintf(stdout, "Executing GPU CSR SpMV kernel complete\n");
    ...
    fprintf(stdout, "GPU ELL SpMV allocated... \n");
    t0 = ReadTSC();
    spmv_gpu_ell(dec, dev, m, n_new, nnz, dex, deb);
    timer[GPU_ELL_TIME] += ElapsedTime(ReadTSC() - t0);

    fprintf(stdout, "Executing GPU ELL SpMV kernel... \n");
    // copy data back from the GPU
...
void print_ell(unsigned int* ell_col_ind, double* ell_vals, 
                      int m, int ell_width) {
    printf("ELL sofar: rows %d, width %d\n", m, ell_width);
    
    printf("value matrix\n");
    for (int i = 0; i < m; i++) {
        printf("\t%d: ", i);
        for (int k = 0; k < ell_width; k++) {
            int idx = i + k * m;  // column-major
            printf(" %.1f, ", ell_vals[idx]);
        }
        printf("\n");
    }
    printf("collumn matrix\n");
    for (int i = 0; i < m; i++) {
        printf("\t%d: ", i);
        for (int k = 0; k < ell_width; k++) {
            int idx = i + k * m;  // column-major
            printf("%u, ", ell_col_ind[idx]);
        }
        printf("\n");
    }
}
...
{	
	...
	for (int i = 0; i < m; i++){
		unsigned int row_nnz = csr_row_ptr[i+1] - csr_row_ptr[i];
	...
	*ell_col_ind = (unsigned int*)malloc(m * max_nnz * sizeof(unsigned int));
    	*ell_vals = (double*)malloc(m * max_nnz * sizeof(double));

   #pragma omp parallel for
    for (int col = 0; col < max_nnz; col++) {
