...
//I am adding functions from last homework
//also two print statements
//
void print_csr(unsigned int* csr_row_ptr, unsigned int* csr_col_ind, double* csr_vals, int m, int nnz) {
    printf("CSR: m=%d, nnz=%d\n", m, nnz);
    printf("Row ptr: ");
    ...
    for (int i = 0; i < 10 && i < nnz; i++) {
        printf("(%u,%.2lf) ", csr_col_ind[i], csr_vals[i]);
    }
    printf("\n");
}

void print_coo(int* row_ind, int* col_ind, double* val, int nnz) {
    printf("COO: nnz=%d\n", nnz);
    printf("First 10: ");
    for (int i = 0; i < 10 && i < nnz; i++) {
        printf("(%d,%d,%.2lf) ", row_ind[i], col_ind[i], val[i]);
    }
    printf("\n");
}

void parallel_histogram_atomic(int* row_ind, int nnz, int m, unsigned int* hist) {
...
void hs_scan(unsigned int* src, unsigned int* prefix, int n)
{   // hillis steele exclusive scan
	
	//fprintf(stderr, "source array:\n<");
	//for (int k = 0; k < n; k++){ fprintf(stderr,"%u, ",  src[k]);}
	//fprintf(stderr, ">\n");

	int * temp = malloc(n * sizeof(unsigned int));
	...
	for (int stride = 1; stride < n; stride *= 2){
		...
		//fprintf(stderr, "After stride %d: ", stride);
        	//for (int k = 0; k < n; k++){ fprintf(stderr,"%u, ",  prefix[k]);}
        	//fprintf(stderr, "\n");
	    }	
...
void b_scan(unsigned int* src, unsigned int* prefix, int n)
{
	//fprintf(stderr, "source array:\n<");
	//for (int k = 0; k < n; k++){ fprintf(stderr,"%u, ",  src[k]);}
	//fprintf(stderr, ">\n");
...

{
    ...
    *csr_vals = calloc(nnz, sizeof(double));
	...
	free(*csr_col_ind);
	free(*csr_vals);
	fprintf(stderr, "memory allocation issues have happened\n");
	return;
    }
    
    //print_coo(row_ind, col_ind, val, nnz);
    //print_csr(*csr_row_ptr, *csr_col_ind, *csr_vals, m, nnz);

    //first make matrix format 0 indexed
    #pragma omp parallel for
    for (int i = 0; i < nnz; i++){
	    row_ind[i]--;
	    col_ind[i]--;
    ...
    #pragma omp parallel
    {
    unsigned int* private_hist = (unsigned int*)calloc(m, sizeof(unsigned int));

    int thread_id = omp_get_thread_num();
     //each thread gets a histogram   
        #pragma omp for 
        for (int i = 0; i < nnz; i++) {
            private_hist[row_ind[i]]++;
	    //printf("  row[%d] = %u elements\n", i, private_hist[i]);
        }
        
        // then they all get merged
        #pragma omp critical
        {
            for (int i = 0; i < m+1; i++) {
                unsigned int old_val = (*csr_row_ptr)[i + 1];
                (*csr_row_ptr)[i + 1] += private_hist[i];
            }
	}	    
	free(private_hist);
    }
    
    //fprintf(stdout, "before b_scan\n");
    //print_csr(*csr_row_ptr, *csr_col_ind, *csr_vals, m, nnz);
    unsigned int* position = calloc((m+1), sizeof(unsigned int));
    
    hs_scan(*csr_row_ptr, position, m+1);
    
	#pragma omp parallel for
    for (int i = 0; i < m; i++) {
        (*csr_row_ptr)[i] = position[i];
    }
    
    for (int i = 0; i < nnz; i++) {
	    int r = row_ind[i];
	    (*csr_col_ind)[position[r]] = col_ind[i];
	    (*csr_vals)[position[r]] = val[i];
	    position[r]++;
	}   
...
{	
    ...
    #pragma omp parallel for reduction(+:res[:m])
    for (int i = 0; i < nnz; i++) {
        unsigned int row = row_ind[i];
                  ...
                  int m, int n, int nnz, double* vector_x, double *res)
{
    ...
    for (int i = 0; i < nnz; i++) {
        res[row_ind[i]] += vals[i] * vector_x[col_ind[i]];
    }
}
